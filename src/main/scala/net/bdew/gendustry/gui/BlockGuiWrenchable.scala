...
trait BlockGuiWrenchable extends Block with IDismantleable {
  ...
  def tryFluidInteract(
    ...
    if (
      activeItem != null && tileEntity != null && tileEntity
        .isInstanceOf[IFluidHandler]
    ) {
      ...
      if (FluidContainerRegistry.isEmptyContainer(activeItem)) {
        ...
        if (fStack != null) {
          val filled =
            FluidContainerRegistry.fillFluidContainer(fStack, activeItem)
          if (filled != null) {
            fluidHandler.drain(
              side,
  ...
  override def onBlockActivated(
    ...
    if (player.isSneaking) {
      ...
      if (
        equipped.isInstanceOf[IToolWrench] && equipped
      ...
      tryFluidInteract(world, x, y, z, player, ForgeDirection.values()(side))
    ) {
      return true
    } else {
      if (!world.isRemote)
        player.openGui(Gendustry.instance, guiId, world, x, y, z)
      return true
    }
  }
}
