...
case class DataSlotErrorStates(name: String, parent: DataSlotContainer)
    extends DataSlotVal[Set[IErrorState]]
    with IErrorLogic {
  ...
  override def getErrorStates: ImmutableSet[IErrorState] =
    ImmutableSet.copyOf(value.toArray)
  ...
  override def readData(data: DataInputStream): Unit = {
    ...
    val errors =
      for (i <- 0 until errNum)
        yield Option(
          ForestryErrorStates.errorStates.getErrorState(data.readUTF())
        )
  ...
  override def load(t: NBTTagCompound, kind: UpdateKind.Value): Unit = {
    value = (t.getList[String](name) flatMap { x =>
      Option(ForestryErrorStates.errorStates.getErrorState(x))
    }).toSet
  }
