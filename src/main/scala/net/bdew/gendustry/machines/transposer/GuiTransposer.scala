...
class GuiTransposer(val te: TileTransposer, player: EntityPlayer)
    extends BaseScreen(new ContainerTransposer(te, player), 176, 166) {
  ...
  override def initGui() {
    ...
    inventorySlots
      .getSlotFromInventory(te, te.slots.inBlank)
      .setBackgroundIcon(HintIcons.sampleOrTemplateBlank)
    inventorySlots
      .getSlotFromInventory(te, te.slots.inLabware)
      .setBackgroundIcon(HintIcons.labware)
    inventorySlots
      .getSlotFromInventory(te, te.slots.inTemplate)
      .setBackgroundIcon(HintIcons.sampleOrTemplate)
  }
}
