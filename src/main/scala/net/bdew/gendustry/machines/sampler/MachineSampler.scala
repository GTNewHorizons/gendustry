...
object MachineSampler
    extends Machine("Sampler", BlockSampler)
    with GuiProvider
  ...
  def getGui(te: TileSampler, player: EntityPlayer): GuiContainer =
    new GuiSampler(te, player)
  def getContainer(te: TileSampler, player: EntityPlayer): Container =
    new ContainerSampler(te, player)
}
