...
class GuiMutatron(val te: TileMutatron, player: EntityPlayer)
    extends BaseScreen(new ContainerMutatron(te, player), 176, 166) {
  ...
  override def initGui() {
    ...
    widgets.add(
      new WidgetLabel(
        Misc.toLocal("gendustry.label.mutatron.title"),
        8,
        6,
        Color.darkGray
      )
