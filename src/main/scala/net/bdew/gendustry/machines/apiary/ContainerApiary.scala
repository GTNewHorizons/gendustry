...
class ContainerApiary(val te: TileApiary, player: EntityPlayer)
    extends BaseContainer(te)
    with ContainerDataSlots
    with ContainerRSControllable {
  ...
  if (!te.getWorld.isRemote && (te.owner :== null)) {
    Gendustry.logInfo(
      "Owner information missing on apiary at (%d,%d,%d), assigning: %s".format(
        te.xCoord,
        te.yCoord,
        te.zCoord,
  ...
  class SlotUpgrade(inv: IInventory, slot: Int, x: Int, y: Int)
      extends SlotValidating(inv, slot, x, y) {
    // Fixes glitch in nei mouse scroll support
    override def isItemValid(stack: ItemStack): Boolean =
      stack == this.getStack || super.isItemValid(stack)
  }

  for (i <- 0 to 1; j <- 0 to 1)
    addSlotToContainer(
      new SlotUpgrade(te, 2 + j + i * 2, 70 + j * 18, 43 + i * 18)
    )

  for (i <- 0 to 2; j <- 0 to 2)
    addSlotToContainer(
      new SlotValidating(te, 6 + j + i * 3, 116 + j * 18, 25 + i * 18)
    )
  ...
  override def transferStackInSlot(
    ...
    if (getSlot(slot).inventory == player.inventory && te.isUpgrade(stack)) {
      ...
      if (canAdd > 0) {
        val remains = ItemUtils.addStackToSlots(
          stack.splitStack(canAdd),
          te,
          te.slots.upgrades,
          true
        )
  ...
  override def slotClick(
    ...
    if (te.slots.upgrades.contains(slot) && te.isUpgrade(oldStack)) {
      ...
      if (te.slots.upgrades.contains(idx) && modifiers == 0 && button <= 1) {
        if (
          te.getStackInSlot(idx) == null || ItemUtils.isSameItem(
            oldStack,
            te.getStackInSlot(idx)
          )
        ) {
