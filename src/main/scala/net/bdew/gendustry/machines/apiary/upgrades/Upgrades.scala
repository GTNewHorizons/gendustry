...
object Upgrades {
  ...
  case class Upgrade(
      ...
      name: String,
      maxNum: Int,
      mods: Seq[(ApiaryModifiers, Int) => Unit]
  ) {
  ...
  def makeNumMod(upg: String, n: String, e: EntryModifier): ModFunc = {
    ...
    n match {
      ...
      case x =>
        sys.error(
          "Unknown numeric upgrade modifier '%s' in upgrade '%s'".format(x, upg)
        )
    }
  }
  ...
  def makeStrMod(upg: String, n: String, v: String): ModFunc = n match {
    ...
    case x =>
      sys.error(
        "Unknown string upgrade modifier '%s' in upgrade '%s'".format(x, upg)
      )
  }
  ...
  def init() {
    for (
      (name, sect) <- Tuning
        .getSection("Upgrades")
        .filterType(classOf[ConfigSection])
    ) {
      ...
      val mods = sect.flatMap({
        ...
        case (pName, v) =>
          sys.error(
            "Unknown upgrade modifier '%s' - %s in upgrade '%s'".format(
              pName,
              v,
              name
            )
      ...
      GameRegistry.registerCustomItemStack(
        "upgrade." + name,
        new ItemStack(ItemApiaryUpgrade, 1, id)
      )
    }
  }
}
