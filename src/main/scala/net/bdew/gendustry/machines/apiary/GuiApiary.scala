...
class GuiApiary(val te: TileApiary, player: EntityPlayer, cont: ContainerApiary)
    extends BaseScreen(cont, 176, 166) {
  ...
  override def initGui() {
    ...
    widgets.add(
      new WidgetApiaryProgress(new Rect(69, 22, 36, 15), te.guiProgress)
    )
    widgets.add(
      new WidgetLabel(
    ...
    te.slots.upgrades.foreach(
      inventorySlots.getSlot(_).setBackgroundIcon(HintIcons.upgrade)
    )
    inventorySlots
      .getSlotFromInventory(te, te.slots.drone)
      .setBackgroundIcon(HintIcons.drone)
    inventorySlots
      .getSlotFromInventory(te, te.slots.queen)
      .setBackgroundIcon(HintIcons.queen)
  }
}
