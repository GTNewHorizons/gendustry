  ...
  def getLocalizedName: String = {
    ...
    val str = allele match {
      ...
      case a: IAlleleTreeSpecies =>
        val k = "for.trees.custom.treealyzer.sapling." + a.getUnlocalizedName
          .replace("trees.species.", "")
        if (Misc.hasLocal(k))
          Misc.toLocal(k)
        else
          a.getName
      case a: IAlleleBeeSpecies =>
        val k = "for.bees.custom.beealyzer.drone." + a.getUnlocalizedName
          .replace("bees.species.", "")
        if (Misc.hasLocal(k))
          Misc.toLocal(k)
        else
          a.getName
      case i: IAlleleInteger =>
        chr match {
          case "GIRTH" => "%d x %d".format(i.getValue, i.getValue)
          case "FERTILITY" if !root.isInstanceOf[ITreeRoot] =>
            i.getValue.toString
          case "METABOLISM" => i.getValue.toString
          case _            => i.getName
        }
      case p: IAllelePlantType =>
        if (p.getPlantTypes.isEmpty) "-"
        else p.getPlantTypes.asScala.mkString(", ")
      case b: IAlleleBoolean =>
        if (b.getValue) Misc.toLocal("gendustry.allele.true")
        else Misc.toLocal("gendustry.allele.false")
      case x => x.getName
    }
    if (str == "")
      return "%s: %s".format(
        Misc.toLocal("gendustry.chromosome." + chr),
        allele.getUID
      )
...
object GeneSampleInfo {
  def fromNBT(t: NBTTagCompound): GeneSampleInfo = {
    ...
    val species =
      AlleleManager.alleleRegistry.getSpeciesRoot(t.getString("species"))
  ...
  def getChromosomeName(root: ISpeciesRoot, chromosome: Int) =
    GeneticsHelper
      .getCleanKaryotype(root)
      .get(chromosome)
      .map(_.toString.toUpperCase(Locale.US))
      .getOrElse("Invalid")
}
