...
class FakeMutatronBeeHousing(tile: TileEntity with IMutatron)
    extends IBeeHousing
    with IBeeModifier
    with IBeeListener
  ...
  override def getCoordinates: ChunkCoordinates =
    new ChunkCoordinates(tile.xCoord, tile.yCoord, tile.zCoord)

  override def getBlockLightValue: Int =
    getWorld.getBlockLightValue(tile.xCoord, tile.yCoord, tile.zCoord)
  override def canBlockSeeTheSky: Boolean =
    getWorld.canBlockSeeTheSky(tile.xCoord, tile.yCoord, tile.zCoord)

  override def getBeeListeners: Iterable[IBeeListener] =
    Collections.singletonList(this)
  override def getBeeInventory: IBeeHousingInventory = this
  override def getBeeModifiers: Iterable[IBeeModifier] =
    Collections.singletonList(this)
  override def getBeekeepingLogic: IBeekeepingLogic =
    beeRoot.createBeekeepingLogic(this)
  ...
  override def setQueen(itemStack: ItemStack): Unit =
    throw new UnsupportedOperationException("Not supported in mutatron")
  override def setDrone(itemStack: ItemStack): Unit =
    throw new UnsupportedOperationException("Not supported in mutatron")
  ...
  override def getProductionModifier(iBeeGenome: IBeeGenome, v: Float): Float =
    v
  override def getMutationModifier(
      iBeeGenome: IBeeGenome,
      iBeeGenome1: IBeeGenome,
      v: Float
  ): Float = v
  ...
  override def getBiome =
    tile.getWorldObj.getBiomeGenForCoordsBody(tile.xCoord, tile.zCoord)
  override def getHumidity: EnumHumidity =
    EnumHumidity.getFromValue(getBiome.rainfall)
  override def getTemperature: EnumTemperature =
    EnumTemperature.getFromValue(getBiome.temperature)

  override def getBeeFXCoordinates: Vec3 = Vec3.createVectorHelper(
    tile.xCoord + 0.5,
    tile.yCoord + 0.5,
    tile.zCoord + 0.5
  )
