...
class Parser
  ...
  def dna = "dna" ~> ":" ~> spec ~ ("=>" ~> int <~ "mb") ^^ { case sp ~ n =>
    RsLiquidDNA(sp, n)
  }
  ...
  def charWithCount = recipeChar ~ ("*" ~> int).? ^^ { case ch ~ cnt =>
    (ch, cnt.getOrElse(1))
  }

  def assembly =
    "assembly" ~> ":" ~> ("id" ~> "=" ~> str <~ ";") ~ (charWithCount <~ ",").+ ~ (int <~ "mj") ~ ("=>" ~> specWithCount) ^^ {
  ...
  def fluidSpec = str ~ (wholeNumber <~ "mb") ^^ { case id ~ amount =>
    FluidSpec(id, amount.toInt)
  }

  def squeezer =
    "squeezer" ~> ":" ~> spec ~ ("," ~> wholeNumber <~ "cycles") ~ ("=>" ~> fluidSpec) ~ ("+" ~> dropsEntry).? ^^ {
      case stack ~ ticks ~ fluid ~ Some((chance, res)) =>
        RsSqueezer(stack, fluid, ticks.toInt, res, chance.round.toInt)
      case stack ~ ticks ~ fluid ~ None =>
        RsSqueezer(stack, fluid, ticks.toInt, null, 0)
    }

  def centrifuge =
    "centrifuge" ~> ":" ~> spec ~ ("," ~> wholeNumber <~ "cycles") ~ ("=>" ~> oneOrManyDrops) ^^ {
  ...
  def stMutation =
    "secret".? ~ ("mutation" ~> ":") ~ (decimalNumber <~ "%") ~ str ~ "+" ~ str ~ "=>" ~ str ~ mutationReq.* ^^ {
      case secret ~ mutation ~ chance ~ p1 ~ plus ~ p2 ~ eq ~ res ~ req =>
        RsMutation(
          parent1 = p1,
          parent2 = p2,
          result = res,
          chance = chance.toFloat,
          secret = secret.isDefined,
          requirements = req
        )
    }
  ...
  def cfgAdd = ident ~ ("+" ~> "=" ~> decimalNumber) ^^ { case id ~ n =>
    CfgVal(id, EntryModifierAdd(n.toFloat))
  }
  def cfgMul = ident ~ ("*" ~> "=" ~> decimalNumber) ^^ { case id ~ n =>
    CfgVal(id, EntryModifierMul(n.toFloat))
  }
  def cfgSub = ident ~ ("-" ~> "=" ~> decimalNumber) ^^ { case id ~ n =>
    CfgVal(id, EntryModifierSub(n.toFloat))
  }
  def cfgDiv = ident ~ ("/" ~> "=" ~> decimalNumber) ^^ { case id ~ n =>
    CfgVal(id, EntryModifierDiv(n.toFloat))
  }
