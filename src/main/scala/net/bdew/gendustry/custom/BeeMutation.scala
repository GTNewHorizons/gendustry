...
class BeeMutation(
    parent1: IAlleleBeeSpecies,
    parent2: IAlleleBeeSpecies,
    result: IAlleleBeeSpecies,
    chance: Float
) extends IBeeMutation {
  ...
  override def getChance(
      housing: IBeeHousing,
      allele0: IAlleleBeeSpecies,
      allele1: IAlleleBeeSpecies,
      genome0: IBeeGenome,
      genome1: IBeeGenome
  ) =
    if (
      !((allele0 == parent1 && allele1 == parent2) || (allele0 == parent2 && allele1 == parent1))
    ) 0
  ...
  override def getSpecialConditions = {
    ...
    reqTemperature.map(x => Misc.toLocalF("gendustry.req.temperature", x)) ++
      ...
      reqBlock.map(x =>
        Misc.toLocalF(
          "gendustry.req.block",
          new ItemStack(x, 1, reqBlockMeta.getOrElse(0)).getDisplayName
        )
      )
  }
