...
class BeeSpecies(cfg: ConfigSection, ident: String) extends IAlleleBeeSpecies {
  ...
  override val getHumidity =
    EnumHumidity.valueOf(cfg.getString("Humidity").toUpperCase(Locale.US))
  override val getTemperature =
    EnumTemperature.valueOf(cfg.getString("Temperature").toUpperCase(Locale.US))
  override val getBranch = AlleleManager.alleleRegistry.getClassification(
    "genus." + cfg.getString("Branch")
  )
  ...
  def prepareLootList(name: String): Map[ItemStack, Float] =
    TuningLoader.loader
      .resolveLootList(cfg.getRaw(name, classOf[EntryLootList]))
      .map(x => x._2 -> x._1.toFloat / 100f)
      .toMap
  ...
  override def getProductChances =
    products.map(x => x._1 -> Float.box(x._2)).asJava
  override def getSpecialtyChances =
    specialty.map(x => x._1 -> Float.box(x._2)).asJava

  override val isNocturnal = cfg.getBoolean("Nocturnal")
  override val getRoot = AlleleManager.alleleRegistry
    .getSpeciesRoot("rootBees")
    .asInstanceOf[IBeeRoot]

  // ==== RESEARCH SYSTEM

  override lazy val getComplexity: Int =
    1 + getMutationPathLength(this, mutable.Set.empty)

  private def getMutationPathLength(
      species: IAlleleSpecies,
      excludeSpecies: mutable.Set[IAlleleSpecies]
  ): Int = {
    var highest = 0
    ...
    getRoot.getPaths(species, getRoot.getKaryotypeKey) foreach { mutation =>
      if (!excludeSpecies.contains(mutation.getAllele0)) {
        val otherAdvance =
          getMutationPathLength(mutation.getAllele0, excludeSpecies)
        if (otherAdvance > highest) highest = otherAdvance
      }
      if (!excludeSpecies.contains(mutation.getAllele1)) {
        val otherAdvance =
          getMutationPathLength(mutation.getAllele1, excludeSpecies)
  ...
  override def getResearchSuitability(itemStack: ItemStack): Float = {
    ...
    if (itemStack == null || itemStack.getItem == null)
      ...
      OreDictionary
        .getOres("beeComb")
        .exists(OreDictionary.itemMatches(_, itemStack, false))
    )
      return 0.4f
    else if (
      OreDictionary
      ...
      return 0.7f
    getRoot.getResearchCatalysts
      .find(x => ItemUtils.isSameItem(x._1, itemStack))
      .foreach(x => return x._2)
    return 0
  }

  override def getResearchBounty(
      world: World,
      researcher: GameProfile,
    ...
    if (world.rand.nextFloat() < 10f / bountyLevel) {
      ...
      if (combinations.nonEmpty)
        res :+= AlleleManager.alleleRegistry.getMutationNoteStack(
          researcher,
          combinations(world.rand.nextInt(combinations.size))
        )
    }

    if (bountyLevel > 10)
      res ++= specialty.keys.map(x =>
        ItemUtils.copyWithRandomSize(x, (bountyLevel / 2f).toInt, world.rand)
      )

    res ++= products.keys.map(x =>
      ItemUtils.copyWithRandomSize(x, (bountyLevel / 2f).toInt, world.rand)
    )
  ...
  def getTemplate = {
    ...
    val tpl = (if (traits.hasValue("Base")) {
                 Option(getRoot.getTemplate(traits.getString("Base")))
                   .getOrElse(
                     sys.error(
                       "Template %s not found".format(traits.getString("Base"))
                     )
                   )
               } else {
    ...
    for (
      (chrName, entry) <- traits.filterType(classOf[EntryStr])
      if chrName != "Base"
    ) {
      ...
      if (!chromosome.getAlleleClass.isInstance(allele))
        sys.error(
          "Allele %s is not valid for chromosome %s".format(
            allele.getUID,
            chromosome
          )
        )
